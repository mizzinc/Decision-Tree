...
DTree.prototype.update_step_numbers_on_form = function () {
    step = 1;
    jQuery(".decision_tree_step_box").not('.ui-sortable-placeholder').each(function (index, element) {
...
jQuery(document).ready(function ($) {
    ...
    //Enable sortable
    $('#dtree_entry_block').sortable({
        beforeStop: function (event, ui) {
            // Update step ids
            var optionList = $(this).find('.decision_tree_next_select option').toArray();
            $(this).children().not('.ui-sortable-placeholder').each(function (stepIndex) {
                var oldStep = parseInt($(this).attr('id').split('-')[1]);
                var step = stepIndex + 1;
                $(this).attr('id', 'decision_tree_step-' + step);
                $(this).children('input[type=hidden]').attr('name', 'raw_step_number_' + step).val(step);
                $(this).children('#decision_tree_choice_block_' + oldStep).attr('id', 'decision_tree_choice_block_' + step);
                $(this).children('.decision_tree_remove_step').attr('data-id', step);
                $(this).find('.decision_tree_remove_choice').attr('data-step_number', step);
                $(this).find('.decision_tree_choice_area input,.decision_tree_choice_area select').removeClass('step_' + oldStep + '_choices').addClass('step_' + step + '_choices');
                $(this).find('.decision_tree_choice_area input[type=hidden]').removeClass('decision_tree_raw_choice_for_step_' + oldStep).addClass('decision_tree_raw_choice_for_step_' + step);
                $(this).find('.decision_tree_add_choice').attr(
                    {
                        step_nuber: step,
                        id: 'dtaddchoice_' + step
                    });
                $(this).find('.decision_tree_cell').each(function () {
                    $(this).children('#decision_tree_step_label-' + oldStep).attr('id', 'decision_tree_step_label-' + step);
                    $(this).find('label[for=' + oldStep + '_type]').attr('for', step + '_type');
                    $(this).find('#' + oldStep + '_type').attr(
                        {
                            id: step + '_type',
                            name: step + '_type',
                            parent_id: 'decision_tree_step-' + step
                        });
                    $(this).find('label[for=' + oldStep + '_question_tex]').attr('for', step + '_question_tex');
                    $(this).find('#' + oldStep + '_question_text').attr(
                        {
                            'data-id': step,
                            id: step + '_question_text',
                            name: step + '_question_text'
                        });
                    $(this).find('label[for=step_' + oldStep + '_textlink]').attr('for', 'step_' + step + '_textlink');
                    $(this).find('#' + oldStep + '_textlink').attr(
                        {
                            id: step + '_textlink',
                            name: step + '_textlink'
                        });
                    $(this).find('label[for=step_' + oldStep + '_subtex]').attr('for', 'step_' + step + '_subtex');
                    $(this).find('#' + oldStep + '_subtext').attr(
                        {
                            id: step + '_subtext',
                            name: step + '_subtext'
                        });
                    $(this).find('label[for=step_' + oldStep + '_info]').attr('for', 'step_' + step + '_info');
                    $(this).find('#' + oldStep + '_info').attr(
                        {
                            id: step + '_info',
                            name: step + '_info'
                        });
                ...
                optionList = optionList.filter(function (option) {
                    ...
                    if (val && parseInt(val) == oldStep) {
                        option.attr('value', step);
                        return false;
                    }
                    return true;
                });
                DTree_field_set.steps[step] = $(this).find('#' + step + '_question_text').val();
            });
