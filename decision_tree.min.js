...
function process_question(dt_id, i) {
    ...
    if (typeof question_history == "undefined") {
        question_history = [];
    }
    ...
    if (window[dtree_name].data[i].type != 'answer') {
        ...
        jQuery("#decision_tree_area_" + dt_id).append("<div id='dt_choice_set_" +
            dt_id + "'>" + option + "</div>");
        if (question_history.length) {
            jQuery("#decision_tree_area_" + dt_id).append('<div class="dt_button question-back"><i class="fa fa-repeat">Back</i></div><br>');
        }
    }
    else {
        //this is an answer
...
jQuery(document).ready(function ($) {
    ...
    jQuery(document).on("click", ".question-back", function () {
        ...
        process_question($(this).closest('form').data('dtid'), question_history[question_history.length - 1]);
    });
});
